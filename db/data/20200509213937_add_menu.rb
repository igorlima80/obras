class AddMenu < SeedMigration::Migration
  def up
    ...
    construcao = MenuItem.create name: 'Gestão de Obras', icon: 'fas fa-archway', link: '#', position: 2, target: '_self', permission: 'manage', permission_check: 'all', menu_item_id: nil, active_test: "", published: true
      ...
      MenuItem.create name: 'Etapas', icon: 'fas fa-bars', link: 'stages_path', position: 2, target: '_self', permission: 'manage', permission_check: 'Stages', menu_item: construcao, active_test: "controller_name == 'stages'", published: true
      MenuItem.create name: 'Sub-Etapas', icon: 'fas fa-tasks', link: 'sub_stages_path', position: 2, target: '_self', permission: 'manage', permission_check: 'SubStages', menu_item: construcao, active_test: "controller_name == 'sub_stages'", published: true
      MenuItem.create name: 'Tipos de Construções', icon: 'fas fa-landmark', link: 'construction_types_path', position: 3, target: '_self', permission: 'manage', permission_check: 'ConstructionType', menu_item: construcao, published: true, active_test: "controller_name == 'construction_types'"     
    ...
    compra = MenuItem.create name: 'Compras e Serviços', icon: 'fas fa-shopping-cart', link: '#', position: 5, target: '_self', permission: 'manage', permission_check: 'all', menu_item_id: nil, active_test: "", published: true
      ...
      MenuItem.create name: 'Materiais', icon: 'fa fa-shopping-basket', link: 'materials_path', position: 7, target: '_self', permission: 'manage', permission_check: 'Material', menu_item: compra, published: true, active_test: "controller_name == 'materials'"     
